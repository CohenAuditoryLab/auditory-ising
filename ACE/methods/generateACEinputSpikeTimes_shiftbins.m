function result_vector = generateACEinputSpikeTimes_shiftbins(data, time_units, bin_size, output_dir)
% generateACEinput - generates *.p file for ACE algorithm from Cohen ...
    ...
    %% LOGIC
        ...
        % shift bins
            ...
            newspikes = zeros(size(spikes_by_bin));
            for i=1:num_neurons
                delay = i+2;
                disp(['Neuron ' num2str(i) ' shifted by ' num2str(delay)]);
                newspikes(i,1:end-delay) = spikes_by_bin(i,1+delay:end);
                newspikes(i, end-(delay-1):end) = spikes_by_bin(i,1:delay);
            end
            spikes_by_bin = newspikes;
        % chunk data
           if (chunk)
